...
const Settings: React.FC = () => {
  ...
  // ===== 主动消息推送服务状态 =====
  const [proactiveServerUrl, setProactiveServerUrl] = useState('');
  const [proactiveUserId, setProactiveUserId] = useState('');
  const [proactiveEnabled, setProactiveEnabled] = useState(false);
  const [proactiveLastCheck, setProactiveLastCheck] = useState(0);

  // 加载主动消息配置
  useEffect(() => {
    const saved = localStorage.getItem('proactiveConfig');
    if (saved) {
      try {
        const cfg = JSON.parse(saved);
        setProactiveServerUrl(cfg.serverUrl || '');
        setProactiveUserId(cfg.userId || '');
        setProactiveEnabled(cfg.enabled || false);
      } catch (e) {}
    }
  }, []);

  // 保存主动消息配置
  const saveProactiveConfig = () => {
    localStorage.setItem('proactiveConfig', JSON.stringify({
      serverUrl: proactiveServerUrl,
      userId: proactiveUserId,
      enabled: proactiveEnabled
    }));
    addToast('推送服务配置已保存', 'success');
  };

  // 主动消息轮询
  useEffect(() => {
    if (!proactiveEnabled || !proactiveServerUrl || !proactiveUserId) return;
    
    const checkMessages = async () => {
      try {
        const baseUrl = proactiveServerUrl.replace(/\/$/, '');
        const url = `${baseUrl}/check-message?user_id=${proactiveUserId}&role_id=all`;
        ...
        if (data.messages && data.messages.length > 0) {
          ...
          if (Notification.permission === 'granted') {
            data.messages.forEach((msg: any) => {
              new Notification('收到新消息', { body: msg.content, icon: '/icon.png' });
            });
          }
          // 确认消息
          const uuids = data.messages.map((m: any) => m.uuid);
          await fetch(`${baseUrl}/ack-message`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({message_uuids: uuids})
          });
          addToast(`收到 ${data.messages.length} 条新消息`, 'info');
        }
        setProactiveLastCheck(Date.now());
      } catch (e) {
        console.error('检查消息失败:', e);
      }
    };
  ...
  }, [proactiveEnabled, proactiveServerUrl, proactiveUserId, addToast]);
    ...
    <div className="h-full w-full bg-slate-50/50 flex flex-col font-light relative">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-20">
        ...
        <section className="bg-white/60 backdrop-blur-sm rounded-3xl p-5 shadow-sm border border-white/50">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                    <div className="p-2 bg-cyan-100/60 rounded-xl text-cyan-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m-5.25 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375M12 18.75a2.25 2.25 0 0 0 2.25-2.25V12m0 0v-.375c0-.621-.504-1.125-1.125-1.125H12a3.75 3.75 0 0 0-3.75 3.75v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                        </svg>
                    </div>
                    <h2 className="text-sm font-semibold text-slate-600 tracking-wider">推送服务（简易版）</h2>
                </div>
            </div>

            <p className="text-xs text-slate-500 mb-3 leading-relaxed">
                连接自己的服务器，接收角色主动发来的消息。需要先在服务器部署推送服务。
            </p>

            <div className="space-y-3">
                <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">服务器地址</label>
                    <input 
                        type="text" 
                        value={proactiveServerUrl} 
                        onChange={(e) => setProactiveServerUrl(e.target.value)} 
                        placeholder="http://101.43.105.195:3000"
                        className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm font-mono"
                    />
                </div>
                <div>
                    <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">用户ID</label>
                    <input 
                        type="text" 
                        value={proactiveUserId} 
                        onChange={(e) => setProactiveUserId(e.target.value)} 
                        placeholder="user_001"
                        className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm"
                    />
                </div>
                <div className="flex items-center gap-2">
                    <input 
                        ...
                        className="w-4 h-4"
                    />
                    <label htmlFor="proactiveEnable" className="text-sm text-slate-600">开启推送接收</label>
                </div>
                <button 
                    onClick={saveProactiveConfig}
                    className="w-full py-3 rounded-2xl font-bold text-white shadow-lg bg-cyan-500 active:scale-95 transition-all"
                >
                    保存配置
                </button>
                {proactiveLastCheck > 0 && (
                    <p className="text-[10px] text-slate-400 text-center">
                        最后检查: {new Date(proactiveLastCheck).toLocaleTimeString()}
                    </p>
                )}
            </div>
        </section>
